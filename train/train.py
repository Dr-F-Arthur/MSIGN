...
if __name__ == '__main__':
    ...
    for repeat in range(repeats):
        ...
        # toy_df = pd.read_csv(os.path.join(data_root, "cb1_ligand_final.csv")).sample(frac=1., random_state=123)
        toy_dir = os.path.join(data_root, 'PDBBind/v2020-other-PL')
        toy_df = pd.read_csv(os.path.join(data_root, "norm.csv")).sample(frac=1., random_state=123)
        # toy_dir = os.path.join(data_root, 'PDBBind/Lppdb')
        # toy_df = pd.read_csv(os.path.join(data_root, "Lppdb.csv"))

        split_idx = int(0.8 * 6000)#int(0.9 * len(toy_df))
        # split_idx = int(11514)
        train_df = toy_df.iloc[:split_idx]
        valid_df = toy_df.iloc[split_idx:6000]
        ...
        for epoch in range(epochs):
            ...
            if valid_rmse < running_best_mse.get_best():
                ...
                if save_model:
                    msg = "epoch-%d, valid_rmse-%.4f, valid_pr-%.4f" \
                    % (epoch, valid_rmse, valid_pr)
