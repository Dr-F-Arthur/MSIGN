...
def mols2graphs(complex_path, label, save_path, dis_threshold=5.0):
    try:
        ...
        g = None
        status = False
    # if status and (not os.path.exists(save_path)):
...
class GraphDataset(object):
    ...
    def __init__(self, data_dir, data_df, dis_threshold=5.0, graph_type='Graph_Atom', num_process=8, create=True):
    ...
    def _pre_process(self):
        ...
        for i, row in data_df.iterrows():
            ...
            # PDBBind
            #cid, pKa = str(row['PDB Code']), float(row['-logKd/Ki'])
